...
```sh
make docker-build docker-push IMG=<some-registry>/opnsense-operator:tag
```
...
```sh
make deploy IMG=<some-registry>/opnsense-operator:tag
```
...
```sh
make build-installer IMG=<some-registry>/opnsense-operator:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/opnsense-operator/<tag or branch>/dist/install.yaml
```
